...
# v0.7.12: LoxBerry MQTT broker authentication is the default protection.
if old_version < 8:
...
printf "%s [INFO] Plugin auf 0.7.12 aktualisiert; Benutzerkonfiguration erhalten\n" "$(date "+%Y-%m-%d %H:%M:%S")" >> "$PLOG/unifipoe.log" 2>/dev/null || true
